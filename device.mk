...
# TODO Later: most of it should work as is. add these later, adjust the values accordingly
# Overlays
...
# TODO Later: these are for specific device, used most of them as is except power profile
PRODUCT_PACKAGES += \
    FrameworksResCommon \
...
# TODO Later: Done each and everyone one. There are few toodo later stuff
# Properties
$(call inherit-product, $(LOCAL_PATH)/properties.mk)

# These are coming from lineage OS, so need not worry much here
# Permissions
PRODUCT_COPY_FILES += \
...
# TODO Later: might be coming from lineage as part of common-qcom.
# Updated few kona references to bengal
# Audio
PRODUCT_PACKAGES += \
...
# TODO Later: This is present in stock rom. hopefully generated in qom-caf
# TODO TEST later: we can get this from stock if required
PRODUCT_PACKAGES += \
...
# These might be coming from lineage. So we can leave them be for now, some are present in stock as well
PRODUCT_COPY_FILES += \
...
# TODO later: all these are replaced with stuff from aospdtgen stuff, check if 
# blah happens...
# init files needs to be same. So can't do much here
# These should be copied automatically I guess as we are using different Android.bp
# Common init scripts
# PRODUCT_PACKAGES += \
#     fstab.qcom \
#     init.class_main.sh \
...
# Dependencies
# TODO Later: may be coming from lineageos. Specified as deps not sure coming from where
PRODUCT_PACKAGES += \
    libavservices_minijail.vendor \
    libhwbinder.vendor

# Display
# These seems to be built from qcom-caf local for display
# and rest of audio etc from qcom-caf from lineage
# We can copy these from stock as well
PRODUCT_PACKAGES += \
...
# These seem to come from qcom-caf some example here
# https://github.com/StormBreakerOSS/platform_hardware_qcom-caf_common/blob/reikon/fwk-detect/src/com/qualcomm/qti/VndFwkDetect.java
# Fwk-detect
PRODUCT_PACKAGES += \
...
# seems same in stock. so leaving it as is
...
# TODO Later: play with these, we have many variants of these in stock. 
# media_codecs_bengal isn't present so copying media_codecs_vendor_v2.xml as bengal and similar with performance
# we can try with bare media_codecs.xml later if required
PRODUCT_COPY_FILES += \
...
# Touch
PRODUCT_PACKAGES += \
    vendor.lineage.touch@1.0-service.capri
...
# TODO Later: check this out for 19.1
